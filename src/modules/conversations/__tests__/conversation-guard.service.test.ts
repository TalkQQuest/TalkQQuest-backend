...
describe("validateReply", () => {
  ...
  // 별도 문장으로 세져서, 자연스러운 1~2문장짜리 답이 too_long으로 거부됐다.
  describe("맨 앞 짧은 감탄사·되물음은 별도 문장으로 세지 않는다(#252)", () => {
    ...
    it("감탄사 + 3문장(총 4조각)까지는 통과한다(감탄사 제외 3문장)", () => {
      expect(
        validateReply("어, 이게? 필기구에 관심 많거든. 어제 새로 산 거야. 너도 궁금하면 빌려줄게.")
      ).toBeNull();
    });

    it("감탄사 + 4문장(총 5조각)은 여전히 거부한다(감탄사 제외 4문장)", () => {
      expect(
        validateReply("어, 이게? 필기구에 관심 많거든. 어제 새로 샀어. 되게 편해. 너도 한번 써볼래?")
      ).toBe("too_long");
    });
