...
- recentMissions(최근 수행한 미션)가 있으면, 그 category와 겹치지 않는 카테고리를 우선적으로
  고릅니다. 같은 장소·상황이 반복되지 않도록 다양성을 의도적으로 추구합니다. 다만 가능한
  카테고리가 이미 recentMissions에서 모두 다뤄졌다면(선택지가 제한적인 경우), 카테고리가
  겹치더라도 구체적인 장소·상대·상황은 다르게 구성해 반복감을 줄입니다.
