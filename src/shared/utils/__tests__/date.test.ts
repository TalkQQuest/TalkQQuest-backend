...
describe("formatDuration", () => {
  ...
  it("mm:ss 형식으로 반환하고, 초는 2자리로 0을 채운다", () => {
    const start = new Date("2026-07-25T10:00:00Z");
    expect(formatDuration(start, new Date("2026-07-25T10:12:34Z"))).toBe("12:34");
    expect(formatDuration(start, new Date("2026-07-25T10:00:05Z"))).toBe("0:05");
  });

  it("초 단위는 버림 처리한다(반올림 아님)", () => {
    const start = new Date("2026-07-25T10:00:00Z");
    // 5분 59.9초 → 5:59 (6:00으로 올림되지 않음)
    expect(formatDuration(start, new Date("2026-07-25T10:05:59.900Z"))).toBe("5:59");
  });

  it("음수(비정상 순서)는 0:00으로 막는다", () => {
    const start = new Date("2026-07-25T10:05:00Z");
    expect(formatDuration(start, new Date("2026-07-25T10:00:00Z"))).toBe("0:00");
  });
});
