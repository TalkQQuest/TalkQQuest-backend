...
const buildSystemPrompt = (ctx: GuideReplyContext): string => {
  ...
  lines.push(
    ...
    "- 사용자의 말이 바로 앞의 질문이나 지금 진행 중인 흐름에 대한 답으로 해석되지 않고, 지금 상황·배역과도 명백히 관련이 없다면(앞선 발화와 연결되지 않는 단답, 갑자기 딴 화제로 이탈 등), 답변의 **첫 문장은 반드시** 그 화제에 대한 짧은 당황·의아함 반응이어야 합니다 (예: \"어, 그게 지금 무슨 말씀이신지..?\", \"네..? 갑자기요?\").",
