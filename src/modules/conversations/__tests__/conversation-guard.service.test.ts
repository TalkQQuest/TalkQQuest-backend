...
describe("validateReply", () => {
  ...
  describe("맨 앞 짧은 감탄사·되물음은 별도 문장으로 세지 않는다 + 상한 4문장(#252)", () => {
    ...
    // #254 리뷰 지적 — 길이만으로 맨 앞 조각을 감탄사로 봤을 때, "네." 같은 평범한
    // 짧은 평서문도 병합돼 진짜 5문장짜리 속사포 답변이 상한(4)을 피해갈 수 있었다.
    // 되물음(?)이거나 흔한 감탄사로 시작할 때만 병합하도록 좁혀 이 경계 케이스를 막는다.
    it("맨 앞이 짧아도 감탄사·되물음이 아닌 평서문이면 병합하지 않는다(5문장은 거부)", () => {
      expect(validateReply("네. 그래요. 좋아요. 알겠어요. 그러네요.")).toBe("too_long");
    });
  });
});
