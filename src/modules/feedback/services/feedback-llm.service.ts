...
- 0~49점: 다음 중 하나에 해당합니다 — (a) 무례하거나 부적절한 발화가 있음, (b) 실질적인 의사 전달이 없는 발화(예: "1", "ㅇㅇ", 의미 없는 자모음, 같은 말을 그냥 반복하는 등 — 짧다는 이유가 아니라 **어떤 상황에 갖다 놔도 뜻이 통하지 않는다는 게 이유**), (c) 미션 맥락과 명백히 무관한 딴 얘기. **이 등급은 그 발화가 해당 지표의 판단 근거에서 상당한 비중을 차지하거나 해당 지표를 직접 훼손할 때만 적용합니다.** 이런 발화가 대화 내내 반복됐다면 40점 이하로, 대화 대부분을 차지했다면 그 비중만큼 45~49점 선까지 올라갈 수 있습니다. 반대로 미션을 정상적으로 잘 수행한 대화 끝에 이런 발화가 한두 번 스친 정도라면, 그 발화와 직접 관련 없는 지표까지 일괄로 이 등급을 주지 않습니다 — 지표별로 그 발화가 실제로 근거를 훼손했는지 따로 판단하세요.
