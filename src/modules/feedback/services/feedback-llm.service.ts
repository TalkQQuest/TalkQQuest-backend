...
- 0~49점: 무례하거나 부적절한 발화가 있거나, 미션 맥락과 명백히 무관한 발화(의미 없는 단답 반복, 대화와 관계없는 딴 얘기 등)가 섞여 있습니다. 무관한 발화의 비중이 클수록 이 범위 안에서도 더 낮은 점수를 줍니다.
- 50~59점: 무례하거나 부적절한 점은 없지만, 그 이상의 노력(먼저 배려를 건네거나, 먼저 대화를 이끌거나, 상대 상황을 헤아리는 등)은 보이지 않는 최소한의 참여입니다.
