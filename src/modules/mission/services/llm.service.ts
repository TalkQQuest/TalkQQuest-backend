...
// 어떤 이유로든(키 없음/HTTP 오류/타임아웃/JSON 깨짐/필드 누락) 실패하면 null을 반환해
// 호출부(recommendMission)가 3단계 템플릿 폴백으로 넘어가게 한다.
...
// LLM 원문 응답 → RecommendedMission. 실패 시 null (호출부가 템플릿 폴백).
export const parseLlmMission = (rawContent: string): RecommendedMission | null => {
...
// 4단계 진입점. 성공 시 RecommendedMission, 실패 시 null(→ 템플릿 폴백).
  ...
  try {
    const res = await fetchWithTimeout(`${env.UPSTAGE_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: buildLlmMessages(context, criteria),
    ...
    return parseLlmMission(content);
  } catch (error) {
    // 타임아웃(AbortError) 포함 모든 네트워크 오류를 폴백으로 흡수한다.
