...
## 4. 에러 코드 = SCREAMING_SNAKE_CASE 문자열 상수

**결정**: 초기에는 `E`(공통), `A`(Auth), `M`(Mission) 등 도메인 접두사 + 4자리 숫자 체계였으나, 기능명세서(기능 정의 v1.0) 반영 이후 `VALIDATION_ERROR`, `UNAUTHORIZED`, `DUPLICATED_EMAIL` 같은 SCREAMING_SNAKE_CASE 문자열 코드로 변경했다. 도메인별 `*.error.ts`에서 공통 `AppError`를 상속하는 구조 자체는 유지.

**왜**: 프론트엔드가 에러를 분기 처리할 때 HTTP status만으로는 부족합니다(예: 402가 "토큰 부족"인지 "결제 실패"인지 구분 필요). 코드 하나로 어떤 도메인의 어떤 에러인지 바로 알 수 있게 하기 위함입니다. 문자열 상수로 바꾼 이유는 접두사+숫자 체계보다 코드 이름만 보고도 의미를 바로 알 수 있고, 기능명세서가 이 형식을 전제로 작성됐기 때문입니다 (design.md `## Error Codes` 참고).
